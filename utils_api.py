...
def get_playlist_musics(sp, playlist_id):
...
def get_playlist_list_musics(sp, lst_playlist_id):

    list_music = []
    for playlist_id in lst_playlist_id:
        list_music.append(get_playlist_musics(sp, playlist_id))
...
def get_audio_features(sp, list_musics_uri):
...
def get_artists(sp, list_artists):
...
def get_df_from_playlist(sp, list_playlist_id, max_nb_artists=None):

    assert isinstance(list_playlist_id, list), 'The input must be a list'
    
    # get all musics from playlist
    print('Getting playlists...')
    list_musics = get_playlist_list_musics(sp, list_playlist_id)
    ...
    audio_features = get_audio_features(sp, df_music.uri.values)
    ...
    # create the is_top50 colunm
    df_music = df_music.assign(is_top50=df_music['popularity'] <= 50)

    # merge the two dataframes
    df_music = df_music.merge(df_audio, on='uri')

    # remove musics without name
    return df_music[~df_music.name.isna()]
